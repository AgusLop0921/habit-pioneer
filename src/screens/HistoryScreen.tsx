...
function getDateStr(d: Date) { return format(d, 'yyyy-MM-dd'); }
...
export default function HistoryScreen() {
  ...
  const CELL = 10, GAP = 2, COLS = Math.floor((SCREEN_W - 48) / (CELL + GAP));
        ...
        {view === 'trends' && (
          <>
            ...
            <View style={s.statsRow}>
              {([
                { icon: 'completed' as IconName, val: totalCompleted, label: t('history.statCompleted') },
            ...
            <View style={s.chartSection}>
              <View style={s.chartHeader}>
                ...
                <Text style={[s.chartAvg, { color: theme.textSecondary }]}>
                  {t('history.avg')} <Text style={{ color: theme.text, fontWeight: '700' }}>{avg}%</Text>
                </Text>
              </View>
              ...
              <View style={s.chartWrap}>
                <Svg width={CHART_W} height={CHART_H + 20}>
                  ...
                  {[0, 50, 100].map(val => (
                    <SvgText
                      key={val}
                      x={0} y={CHART_H - (val/100) * CHART_H + 5}
                  ...
                  {[0, 50, 100].map(val => (
                    <Line
                      key={val}
                      x1={32} y1={CHART_H - (val/100) * CHART_H}
                      x2={CHART_W} y2={CHART_H - (val/100) * CHART_H}
                  ...
                  {dataPoints.map((val, i) => {
                    ...
                    return (
                      <SvgCircle key={i} cx={x} cy={y} r={4} fill={theme.accent} />
                    );
                  })}
                </Svg>
                ...
                <View style={s.xLabels}>
                  {(range === '7D' ? days : days.filter((_, i) => i % Math.floor(days.length/6) === 0))
                    .slice(0, 7)
                    .map((d, i) => (
              ...
              <View style={s.rangeRow}>
                {rangeOptions.map(r => (
                  <Pressable
                    ...
                    onPress={() => setRange(r)}
                  >
                    <Text style={[s.rangeBtnText, { color: range === r ? theme.text : theme.textSecondary }]}>
                      {r}
                    </Text>
                  </Pressable>
                ))}
                {/* Heatmap toggle */}
                <Pressable
                  style={[s.rangeBtn, { backgroundColor: range === '12M' ? theme.surface2 : 'transparent' }]}
                >
            ...
            <Text style={[s.empty, { color: theme.textSecondary }]}>{t('history.noHabits')}</Text>
              ...
              const streak = (() => {
                ...
                for (let i = 0; i < 365; i++) {
                  ...
                  if (history[d]?.[h.id]) s++; else break;
                }
                return s;
              })();
              const longest = (() => {
                ...
                for (let i = 364; i >= 0; i--) {
                  const d = getDateStr(subDays(today, i));
                  if (history[d]?.[h.id]) { cur++; max = Math.max(max, cur); } else cur = 0;
                }
                return max;
              })();
              ...
              return (
                <View key={h.id} style={[s.habitHistCard, { backgroundColor: theme.surface, borderColor: theme.borderDim }]}>
                  <View style={s.habitHistHeader}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={[s.habitHistStats, { color: theme.textSecondary }]}>
                        {t('history.streak')}: <Text style={{ color: theme.text, fontWeight: '700' }}>{streak}</Text>
                        {'  '}{t('history.longest')}: <Text style={{ color: theme.text, fontWeight: '700' }}>{longest}</Text>
                        {'  '}{t('history.completed')}: <Text style={{ color: theme.text, fontWeight: '700' }}>{completed}</Text>
                      </Text>
                    </View>
                  </View>
                  {/* Mini 7-day bar */}
                  <View style={s.miniBarRow}>
                  {(t('history.weekDaysShort', { returnObjects: true }) as string[]).map((day, i) => {
                      const d = last7[i];
                      const dateStr = getDateStr(d);
                      ...
                      return (
                        <View key={i} style={s.miniBarCol}>
                          <View style={[
                            ...
                            isToday && isDone && { backgroundColor: theme.accent, height: 48 },
                          ]} />
                          <Text style={[s.miniBarLabel, { color: theme.textSecondary }]}>{day}</Text>
                        </View>
                      );
                    })}
                  </View>
          ...
          <View style={s.chartSection}>
            <View style={s.chartHeader}>
              <View>
                <Text style={[s.chartTitle, { color: theme.text }]}>{t('history.chartTitle')}</Text>
                <Text style={[s.chartSub, { color: theme.textSecondary }]}>
                  {t('history.days365')} · {format(subDays(today, 364), 'd MMM yyyy')} – {format(today, 'd MMM yyyy')}
                </Text>
              </View>
              <Text style={[s.chartAvg, { color: theme.textSecondary }]}>
                {t('history.avg')} <Text style={{ color: theme.text, fontWeight: '700' }}>{avg}%</Text>
              ...
              {heatmapDays.map((d, i) => {
                ...
                return (
                  <View
                    ...
                    style={[
                      s.heatCell,
                      { backgroundColor: `rgba(124,92,252,${opacity})` },
                    ]}
                  />
                );
              })}
...
const makeStyles = (theme: any) => StyleSheet.create({
  ...
  habitHistHeader: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: Spacing.md },
  habitHistEmoji: { fontSize: 28 },
  habitHistName: { fontSize: 15, fontWeight: '600' },
  habitHistStats: { fontSize: 12, marginTop: 2 },
  miniBarRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', height: 56 },
