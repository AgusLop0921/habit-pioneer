...
export default function MotivationalScreen({ onContinue }: Props) {
  ...
  const handleContinue = () => {
    Animated.timing(screenOpacity, { toValue: 0, duration: 350, useNativeDriver: true }).start(() => onContinue());
  };
  ...
  return (
    <Animated.View style={[styles.container, { opacity: screenOpacity, backgroundColor: theme.bg }]}>
      <Pressable style={styles.inner} onPress={handleContinue}>
        <Animated.Text style={[styles.emoji, { transform: [{ translateY: floatAnim }] }]}>✨</Animated.Text>
        <View style={[styles.line, { backgroundColor: theme.orange }]} />
        <Animated.Text style={[styles.quote, { color: theme.text, opacity, transform: [{ translateY }] }]}>
          "{quote.text}"
        </Animated.Text>
...
const styles = StyleSheet.create({
  ...
  quote: { fontSize: 22, fontWeight: '700', textAlign: 'center', lineHeight: 32, letterSpacing: -0.3 },
  author: { fontSize: 13, marginTop: Spacing.lg, fontWeight: '500', letterSpacing: 0.5, textTransform: 'uppercase' },
  tap: { position: 'absolute', bottom: 56, fontSize: 12, letterSpacing: 1, textTransform: 'uppercase' },
});
