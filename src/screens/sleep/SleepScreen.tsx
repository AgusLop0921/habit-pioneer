...
export default function SleepScreen() {
  ...
  if (screen === 'onboarding') {
    return <SleepOnboarding onDone={() => setScreen('main')} />;
  }
      ...
      <SafeAreaView style={[s.safe, { backgroundColor: theme.bg }]} edges={['top']}>
        ...
        <ScrollView contentContainerStyle={s.enrollWrap} showsVerticalScrollIndicator={false}>
          ...
          {[
            ...
            <View
              key={i}
              style={[s.benefit, { backgroundColor: theme.surface, borderColor: theme.borderDim }]}
            >
              <View style={[s.benefitIcon, { backgroundColor: `${INDIGO}15` }]}>
                <Icon name={b.icon as IconName} size={18} color={INDIGO} />
              </View>
            ...
            {[
              ...
              >
                <View style={[s2.statIcon, { backgroundColor: `${st.color}15` }]}>
                  <Icon name={st.icon as IconName} size={18} color={st.color} />
                </View>
                ...
                <Svg width={CHART_W} height={CHART_H + 10} style={{ marginTop: 8 }}>
                  {[4, 6, 8].map((h) => (
                    <Line
                      key={h}
                      x1={0}
                      y1={CHART_H - (h / maxH) * CHART_H}
                      x2={CHART_W}
                      y2={CHART_H - (h / maxH) * CHART_H}
                    ...
                    <Polyline
                      points={chartData
                        .map((d, i) => {
                          ...
                          const x =
                            i * (CHART_W / chartData.length) + CHART_W / chartData.length / 2;
...
const makeMainStyles = (_theme: AppTheme) =>
  StyleSheet.create({
    header: {
      flexDirection: 'row',
      alignItems: 'center',
