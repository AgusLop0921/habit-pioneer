...
# ============================================================
#  Habits Pioneer — Patch v6 — SwipeableRow iOS nativo
#
#  Fix: las acciones viven DENTRO de la card redondeada.
#  Solo el contenido se mueve, las acciones quedan fijas
#  en el lado derecho — mismo borderRadius, sin conflicto.
#
#  Correr DENTRO de habits-pioneer/:
#    bash patch-v6-swipe.sh
# ============================================================
...
echo -e "${CYAN}"
echo "  ╔══════════════════════════════════════════╗"
echo "  ║  Patch v6 — Swipe iOS nativo             ║"
echo "  ╚══════════════════════════════════════════╝"
echo -e "${NC}"

[ -f "app.json" ] || { echo "Corré desde habits-pioneer/"; exit 1; }

step "Reescribiendo SwipeableRow — patrón iOS nativo"

cat > src/components/common/SwipeableRow.tsx << 'SWIPE_EOF'
/**
...
export default function SwipeableRow({
  ...
  const handleDelete = () => { snapTo(0, () => setTimeout(onDelete, 50)); };
  const handleEdit   = () => { snapTo(0, () => setTimeout(onEdit,   50)); };
...
log "SwipeableRow reescrito ✓"

echo ""
echo -e "${GREEN}╔═══════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  ✓  Patch v6 listo                                ║${NC}"
echo -e "${GREEN}╠═══════════════════════════════════════════════════╣${NC}"
echo -e "${CYAN}║  Cómo funciona ahora:                             ║${NC}"
echo -e "${CYAN}║  • La card completa es el contenedor redondeado   ║${NC}"
echo -e "${CYAN}║  • Las acciones viven dentro (absoluteFill)       ║${NC}"
echo -e "${CYAN}║  • Solo el contenido se desliza con translateX    ║${NC}"
echo -e "${CYAN}║  • overflow:hidden recorta todo al borderRadius   ║${NC}"
echo -e "${CYAN}║  → Sin bordes dobles, sin conflicto visual        ║${NC}"
echo -e "${GREEN}╠═══════════════════════════════════════════════════╣${NC}"
